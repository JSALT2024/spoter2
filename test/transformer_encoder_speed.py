...
def test_seq_first(number: int = 1000):
    ...
    print(f"Data: [s, b, f], Transformer: batch_first=False, Time: {t / number:.3f}s")
...
def test_batch_first(number: int = 1000):
    ...
    t = timeit.timeit(lambda: transformer_encoder(x), number=number)
    print(f"Data: [b, s, f], Transformer: batch_first=True, Time: {t / number:.3f}s")
...
def test_seq_first_permute(number: int = 1000):
    ...
    print(f"Data: [b, s, f] + permute(1,0,2), Transformer: batch_first=False, Time: {t / number:.3f}s")


if __name__ == "__main__":
    """
    runs = 1000
    Data: [s, b, f], Transformer: batch_first=False, Time: 0.027s
    Data: [b, s, f], Transformer: batch_first=True, Time: 0.026s
    Data: [b, s, f] + permute(1,0,2), Transformer: batch_first=False, Time: 0.030s
    """
    runs = 1000
    print(device)
